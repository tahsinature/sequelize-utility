...
module.exports = (table, db) => {
  return new Promise(async (res, rej) => {
    ...
    if (!db)
      return rej(new Error("Please provide a definite sequelize instance."));
    ...
    if (!isAllFromProvidedDB)
      return rej(new Error("Please provide all models from same db."));

    for (const m of models) {
      if (!tables.includes(m)) {
        await m.sync({ force: true });
        debugDB(`[Restored]-> ${m.name}`);
      } else debugDB(`[Skipped]-> ${m.name}`);
    }
    res();
  });
};
